...
public final class EglCore {
    ...
    public EglCore(EGLContext sharedContext, int flags) {
        ...
        if (!EGL14.eglInitialize(mEGLDisplay, version, 0, version, 1)) {
            mEGLDisplay = null;
            throw new RuntimeException("unable to initial EGL14");
        }
    ...
    @Override
    protected void finalize() throws Throwable { //在释放资源是做的额外保护
        try {
            if (mEGLDisplay != EGL14.EGL_NO_DISPLAY) {
    ...
    public EGLSurface createWindowSurface(Object surface) {
        ...
        //将Surface（是ANativeWindow） 转换为本地窗口 （mEglSurface）
    ...
    public void makeCurrent(EGLSurface eglSurface) {
        ...
        //绑定EglContext和Surface到显示设备(mEGLDisplay)中, 第二个参数表示draw 绘制，第三个参数表示read 读
        // 7、将EGLDisplay、EGLSurface和EGLContext进行绑定（渲染上下文绑定到渲染面, 指定当前的环境为绘制环境 EGLContext->context）
        // eglMakeCurrent后生成的surface （上面生成的mEglSurface）就可以利用opengl画图了
    ...
    public static void logCurrent(String msg) {
        ...
        Log.i(TAG,"Current EGL ("+msg+"): display="+eglDisplay+", context="+eglContext+
                ", surface="+eglSurface);
    }
}
